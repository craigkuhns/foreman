...
<div class="input">
  <?php if ($editable == true) { ?>
    ...
    <input data-input-id="<?php echo foreman_field_id($field) ?>" data-use-as-label="<?php echo $field['name'] ?>" class="button upload" type="button" value="Upload <?php echo $field['name'] ?>" />
    <?php if (!empty($field['description'])) { ?>
      <p class='foreman-field-description'><?php echo $field['description'] ?></p>
    <?php } ?>
  <?php } else { ?>
    <p class="foreman-uneditable-form-value"><a href="<?php echo $meta ?>" target="_blank">Open file</a></p>
  <?php } ?>
</div>